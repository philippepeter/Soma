 ...
 * (PropertyChangeListener) et notifie les changements d'état de l'algorithme �
 * l'ApplicationFrame.
 * 
 * @author Pip
 * 
...
public class AlgorithmManager implements BacktrackerListener,
	...
	private final static String DEFAULT_TEXT = "Choisissez un puzzle et lancez la résolution via le menu, vous pouvez faire tourner les piéces et le puzzle avec la souris.";
	...
	protected void resolveAll() {
		if (frame.getCurrentSoma() != null) {
			...
			// Le nombre de solutions est trés grand on echantillonne tous les
			// 100000.
			samplingRate = 100000;
	...
	private void startAlgo(boolean stopOnSolutionFound) {
		...
		if (data.getImpossibleShapesCount() != 0) {
			JOptionPane.showMessageDialog(frame, data
					...
					+ " piéces ne rentrent pas dans le puzzle",
